  ...
  return (
    <div className="flex flex-col h-full bg-white">
      <header className="px-4 py-3 border-b border-gray-200">
        <h1 className="text-base font-semibold text-gray-900">Detections</h1>
      </header>
      ...
      <div className="flex-1 overflow-auto p-4">
        <table className="min-w-full text-left text-sm font-mono border-collapse">
          ...
          <tbody>
            {data?.vision_results?.map((item, idx) => (
              <tr key={idx} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                <td className="px-4 py-2">element</td>
                <td className="px-4 py-2">{Math.round((item.confidence || 1) * 100)}%</td>
                <td className="px-4 py-2">{Math.round(item.bbox[0])}</td>
                <td className="px-4 py-2">{Math.round(item.bbox[1])}</td>
                <td className="px-4 py-2">{Math.round(item.bbox[2])}</td>
                <td className="px-4 py-2">{Math.round(item.bbox[3])}</td>
                <td className="px-4 py-2 truncate max-w-xs">{item.text_content || '-'}</td>
              </tr>
            ))}
            {!data?.vision_results?.length && (
              <tr>
                <td colSpan={7} className="text-center py-8 text-gray-500 italic">No detections available.</td>
              </tr>
            )}
          </tbody>
        </table>

...
async def main():
    ...
    if not webapp_ready:
        logger.error("ERROR: Web app server did not start in time")
        webapp_proc.terminate()
        gateway_proc.terminate()
        return

    if not gateway_ready:
        logger.error("ERROR: Gateway server did not start in time")
        webapp_proc.terminate()
        gateway_proc.terminate()
        return

    logger.info(f"[OK] Web App running at http://127.0.0.1:{WEBAPP_PORT}")
    logger.info(f"[OK] Gateway API running at http://127.0.0.1:{GATEWAY_PORT}")
    ...
    try:
        ...
        logger.info("Pausing 5 seconds for visual inspection of the Chromium browser window...")
        ...
        logger.info("Cleaning up demo processes...")
        webapp_proc.terminate()
        gateway_proc.terminate()
